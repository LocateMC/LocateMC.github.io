---
title: '九韵万象图'
author: '那年那兔哪些事儿'
summary: '徽派、佛教、中式古建、道教、神话等九大玄方幻区域，建筑块累计超 300 亿的国风巨型建筑地图。'
href: 'https://1835870117.share.123pan.cn/123pan/IK9xTd-Rbbkd?pwd=1145'
source: 'https://www.bilibili.com/video/BV11Y8Jz4EUZ/'
icon: '/icons/sample.webp'
screenshot: '/screenshots/jywxt.jpg'
type: '地图及投影'
edition: 'Java'
loaders: []
versions: []
tags: ['徽派建筑', '佛教建筑', '中式古建筑', '道教建筑', '神话建筑', '寺庙建筑', '建筑地图', '国风', '中式']
order: 1
---

九韵万象图是一张以中国传统建筑与神话意象为题材的 Java 版巨型建筑地图，将徽派建筑、佛教建筑、中式古建筑、道教建筑、神话建筑、寺庙建筑等九大玄方幻区域融为一体，建筑方块累计超过 300 亿。整张地图以不同主题区域的拼合方式呈现传统建筑美学与神话空间的张力，适合走读、鉴赏与摄影。